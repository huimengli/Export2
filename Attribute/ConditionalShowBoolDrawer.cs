...
namespace Export.Attribute
{
    ...
    public class ConditionalShowBoolDrawer : PropertyDrawerEX
    {
        /// <summary>
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            ConditionalShowBoolAttribute condHAtt = (ConditionalShowBoolAttribute)attribute;
            // 查找条件源字段
            SerializedProperty sourceProperty = property.serializedObject.FindProperty(condHAtt.ConditionalSourceField);

            // 检查条件是否满足显示要求
            bool show = ShouldShow(sourceProperty, condHAtt.ShowValue);
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            // 条件不满足时高度设为负值（完全隐藏）
            if (!ShouldShow(sourceProperty, condHAtt.ShowValue))
            {
        ...
        /// <returns></returns>
        private bool ShouldShow(SerializedProperty sourceProperty, bool showValue)
        {
            // 检查源字段是否存在且是否为枚举类型
            ...
            // 获取当前枚举值索引
            bool currentValue = sourceProperty.boolValue;
            // 检查当前值是否在允许显示的数值列表中
            if (currentValue == showValue) return true;
